...
def testing_page():
    ...
    with col2:
        ...
        create_session = st.checkbox(
            "Utwórz nową sesję przed testem", value=True, key="create_test_session"
        )

        # Clear logs option
        clear_logs = st.checkbox("Wyczyść logi przed testem", value=True, key="clear_test_logs")
...
def run_automated_test(test_params):
    ...
    if clear_history:
        ...
        add_technical_log(
            "info", "🧹 Historia konwersacji wyczyszczona - test zaczyna od czystego stanu"
        )
    ...
    add_technical_log(
        "info", f"🧪 Test '{test_name}' rozpoczęty - {len(responses_list)} odpowiedzi"
    )
    ...
    add_technical_log(
        "info", f"🧪 Test zakończony - przetestowano {st.session_state.test_progress} odpowiedzi"
    )
    ...
    test_results = {
        ...
        "test_summary": {
            ...
            "stage_transitions": len(
                [
                    log
                    for log in st.session_state.get("technical_log", [])
...
def export_test_logs():
    ...
    st.download_button(
        label="⬇️ Pobierz JSON", data=json_str, file_name=filename, mime="application/json"
    )
...
def copy_logs_to_clipboard():
    ...
    st.text_area("Logi do skopiowania:", value=logs_text, height=300, key="copyable_logs")

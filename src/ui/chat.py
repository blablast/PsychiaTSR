        ...
        if tts_config and tts_config.get("api_key") and tts_config.get("voice_id"):
            ...
            if prefer_webrtc:
                ...
                try:
                    ...
                    add_technical_log("audio_status", f"🎙️ WebRTC aktywne (Voice: {tts_config.get('voice_id', 'unknown')})")
                    ...
                    with st.expander("🔧 Alternatywne rozwiązania audio", expanded=False):
                        st.info("""💡 **WebRTC nie działa? Wypróbuj te rozwiązania:**
...
Przełącz na HTML5 Audio w Ustawieniach - nie wymaga mikrofonu.
                ...
                add_technical_log("audio_status", f"🎙️ HTML5 Audio aktywne (Voice: {tts_config.get('voice_id', 'unknown')})")
            ...
            if 'conversation_manager' in st.session_state:
                ...
                for message in conversation:
                    ...
                    with st.chat_message(message.role, avatar=avatar):
                        ...
                        with col_audio:
                            if message.role == "therapist" and hasattr(message, 'audio_file_path') and message.audio_file_path:
                            ...
                            if response_times:
                                ...
                                if sup_time > 0 or ther_time > 0:
                                    ...
                                    if first_chunk_time is not None:
                                        time_parts.append(f"1st: {first_chunk_time/1000:.2f}s - {(sup_time+first_chunk_time)/1000:.2f} from start")
                                    if ther_time > 0:
        ...
        if (
            ...
            with streaming_container:
                with st.chat_message("therapist", avatar="💚"):
                    ...
                    if (audio_service and
                        st.session_state.get("audio_enabled", False) and
                        st.session_state.get("_tts_cfg")):
                    ...
                    if tts_router:
                        ...
                        add_technical_log("audio_tts", f"🎵 TTS WebRTC zakończone - wygenerowano audio dla {len(full_response)} znaków")
                    elif use_html5_audio and accumulated_text.strip():
                        # Generate and save audio file (no auto-player, just for 🔊 button)
                        add_technical_log("audio_debug", f"🎵 Generuję audio dla przycisku play: {len(accumulated_text)} znaków")
...
def _process_and_save_audio(text: str, tts_config: dict) -> None:
    ...
    try:
        add_technical_log("audio_debug", f"🎵 Rozpoczynam generowanie i zapisywanie audio dla {len(text)} znaków")
        ...
        if not tts_provider.is_available():
            add_technical_log("audio_error", f"❌ ElevenLabs niedostępne - API key: {'tak' if tts_config.get('api_key') else 'nie'}")
            return
        ...
        add_technical_log("audio_debug", f"🎵 Audio wygenerowane, rozmiar: {len(audio_data)} bajtów")
        ...
        if audio_file_path:
            ...
            add_technical_log("audio_tts", f"🎵 Audio zapisane: {audio_file_path} ({len(audio_data)} bajtów)")
...
def _get_response_times_for_message(message):
    ...
    try:
        ...
        first_chunk_time = entry_data.get('first_chunk_time_ms') if isinstance(entry_data, dict) else None
